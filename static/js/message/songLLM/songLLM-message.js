...
// 메세지 전송 함수
// var sendMessageFun = async (clientMessageText, serverMessageText) => {
var sendMessageFun = async (clientMessageText) => {
  ...
  // 클라이언트가 서버로 메시지를 전송하고 서버 응답을 처리하는 비동기 함수
  await songMessageService.clientSend(clientMessageText).then((serverMessage) => {
    // 서버로부터 받은 응답에서 'serverMessageText'를 추출
    let serverMessageText = serverMessage.serverMessageText
